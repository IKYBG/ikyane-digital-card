...
export default async function PublicProfilePage({ params }: { params: Promise<{ slug: string }> }) { const { slug } = await params; let data; try { data = await getPublicQard(slug); } catch { return <main className="public-qard-error"><b>Qard</b><h1>Configuration en cours.</h1><p>Le service de profils sera disponible dès que Supabase sera connecté.</p></main>; } if (!data) notFound(); if (data.profile.slug !== slug) redirect(`/u/${data.profile.slug}`); return <PublicQard data={data} />; }
