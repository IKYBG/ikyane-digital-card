...
export default function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    ['u_color1', 'u_color2', 'u_color3'].forEach((name, index) =>
      gl.uniform4fv(uniform(name), colors[index]),
    );
    gl.uniform1f(uniforms.scale, config.scale ?? 0.46);
    ...
    gl.uniform1f(
      ...
      Math.min(config.swirlIterations ?? 3, 4),
    );
    ...
    const draw = (now: number) => {
      if (now - previousFrame >= frameDuration || reducedMotion) {
        ...
        gl.uniform1f(
          uniforms.time,
          ((now - startedAt) / 1000) * ((config.speed ?? 13) / 100) * 5 +
            (config.offset ?? -180) * 0.01,
        );
      ...
      {noise && noise.opacity > 0 ? (
        <div
          ...
          style={{
            ...
            backgroundSize: `${(noise.scale ?? 1) * 180}px`,
          }}
        />
      ) : null}
    </div>
